...
def _run_builder(model_slug: str, max_steps: int | None) -> None:
    if model_slug in _DATAMART_MODELS:
        ...
        build.main(getattr(build, _DATAMART_MODELS[model_slug]))
    elif model_slug == "hrdps-west":
        from . import build_1km

        build_1km.main()
    elif model_slug == "hrrr-conus":
        from . import build_hrrr

        build_hrrr.main()
    elif model_slug == "gfs":
        from . import build_gfs

        build_gfs.main()
    elif model_slug in {"nam", "nam-conus-nest"}:
        from . import build_nam

        build_nam.build(build_nam.PRODUCTS[model_slug])
    elif model_slug in {"reps", "geps"}:
        module = __import__(f"windgram.build_{model_slug}", fromlist=["main"])

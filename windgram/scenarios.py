...
from .windgram import derive_windgram_profile
...
def _validate_profile(
    ...
    scenario_id = definition["id"]
    schema_path = repository_root / "packages" / "windgram" / "schema" / "profile.schema.json"
...
def generate_scenario(
    ...
    if definition.get("kind") == "comparison":
        raise ScenarioError(
            ...
            "`python -m windgram.scenarios generate`"
        )
    validate_definition(
        definition,
...
def check_repository(*, repository_root: Path = REPOSITORY_ROOT) -> None:
    ...
    if problems:
        raise ScenarioCheckError(
            ...
            + "\n  ".join(problems)
            + "\nrun `uv run python -m windgram.scenarios generate`"
        )
...
def _resolve_repository_root() -> Path:
    ...
    for candidate in (cwd, *cwd.parents, REPOSITORY_ROOT):
        if (candidate / "scenarios" / "scenario.schema.json").is_file() and (
            candidate / "packages" / "windgram" / "schema"
        ).is_dir():
            return candidate

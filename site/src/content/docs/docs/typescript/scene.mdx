...
- `launch: { elevationM }` draws the launch line at that elevation, labelled
  ...
  [site-context.json](/docs/reference/site-context-document/).
- No `launch` option → `scene.launch` is null and no marker draws. That
  absence is honest — the document never knew a launch — and it is not an
  error.
...
measurement strips — renders below a labeled divider (*"beside this model — not in its

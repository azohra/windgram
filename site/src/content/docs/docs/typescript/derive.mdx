...
if (!isSmokeAwareProfile(profile)) {
  const smokeByHour = smokeHoursByValidAt(smokeDocument);
  const hour = profile.hours[i];
  const joined = smokeByHour.get(hour.validAt);
  if (joined) {
    const aot = smokeAotFromColumn(p50(joined.smokePlumeColumnMgm2)!);
    const f = smokeTransmittance(
      aot,
      cosSolarZenith(hour.validAt, profile.site.latitude, profile.site.longitude),
    );
    const adjusted = smokeAdjustedThermalVelocityMs(p50(hour.derived.thermalVelocityMs)!, f);
  }
}
```

...
documents. It does not duplicate pipeline derivations that require raw model
inputs.
...
| Owner | Quantities |
| --- | --- |
| Python pipeline | Published boundary-layer top, thermal velocity, cloud base, and default usable-lift top |
| TypeScript derive export | Moisture conversions, vector wind, lapse and stability, thermal index, shear, B/S ratio, local-day grouping, projection, valid-time alignment, units, smoothing, and parameterized usable lift |

The package's `usableLiftTopM(inputs, sinkRateMs)` projects the published
inputs for another sink rate without replacing the stored default.
At `1.0` m/s it matches the pipeline fixture exactly. The scene does not apply
that p50 recomputation to ensembles because it would not equal a per-member
...
Profiles publish all forecast hours in UTC. Profiles from the v0.4 publication
wave echo the catalogued IANA timezone at `site.timeZone`; an older compatible
profile may omit it, so callers still need an explicit fallback.

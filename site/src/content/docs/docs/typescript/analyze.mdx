...
Since 0.22, everything a downstream comparison validates or states about a
member is **on the envelope**, so a serialized `WindgramAnalysis` re-enters
[`compareAnalyses`](/docs/typescript/compare/#compare-cached-analyses)
...
- `extensions` — named third-party statements, when
  ...
  empty**, otherwise, so pre-0.22 serialized envelopes are byte-identical.

These are required fields, which is additive for every *reader* of the
envelope — only code that constructs `WindgramAnalysis` values by hand
(test fixtures) gains fields to fill. Analyze once at the edge, cache the
envelope as JSON, and compare later: the self-description is what makes
the cached workflow honest.

## Versioning reads tolerantly

`vocabularyVersion` is typed `number`, not the version literal — the one
type-level break of the 0.22 release, with zero wire change. It encodes
...
export function dayVerdicts(envelope: WindgramAnalysis) {
  if (envelope.vocabularyVersion > ANALYZE_VOCABULARY_VERSION) {
    // A newer toolkit produced this envelope. Additive kinds are the
...
[that fact's one home](/docs/reference/smoke-document/#the-column-field-carries-a-provider-defect)),
...
extractor stands on — is public since 0.22: `AnalysisFrame`, versioned
...
available through `frame.profile`; `windgram/derive` exports the same
selectors the first-party extractors use (`p50`, `localDateKey`,
`groupByLocalDay`).

```ts title="window-pace-extension.ts"
import { analyzeProfile, type AnalysisExtension } from "windgram/analyze";
...
export function paceStatements(profile: WindgramProfile): WindowPaceStatement[] {
  const analysis = analyzeProfile(profile, { extensions: [windowPace] });

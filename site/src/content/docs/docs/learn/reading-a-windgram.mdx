...
| **Smoke · µg/m³** | Near-surface wildfire smoke, with a haze tint that deepens with the model's own published optical depth. The label names the smoke model — it can be a different model than the profile's; a joined smoke document draws no tint (its column is quarantined). | Surface air can be clean under an elevated plume (line low, tint deep) and vice versa; see [Smoke and thermals](/docs/learn/smoke-and-thermals/). |
...
a stack of observed layers. The launch marker is supplied by the renderer, not the document — the
consumer passes a launch elevation at render time, normally the measured `elevation` pick from
[site-context.json](/docs/reference/site-context-document/); a render given no launch honestly
draws no marker. Surface values are referenced to the model's own smoothed terrain —

...
/* The multi-model comparison overlay — boundary-layer top and usable-lift
   top for every model that covers the chosen day, one colour per model,
   with a min-max spread band. This is a SITE feature over many profiles at
   once, not part of the windgram package (whose scene renders one model);
   the app extracts plain-number series from profiles (p50 for
   ensembles) before they arrive here. */
...
export function renderOverlaySVG(series: OverlaySeries[]): SVGSVGElement {
  ...
  for (const { s, blPts, liftPts } of prepared) {
    ...
    if (labelPts.length) {
      ...
      pendingLabels.push({ x: lx + 8, y: ly + 3, color: s.color, text: s.label });
    }
  }

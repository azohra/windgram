"""NOAA Open Data GRIB2 access shared by the HRRR, GFS, and NAM builders.
...
USER_AGENT = "Windgram/2.0 (+https://github.com/azohra/windgram)"
...
class MissingRecordError(RuntimeError):
    """The .idx has no such record — distinct from transport failures so
    optional-field tasks can tolerate absence without masking real errors."""
...
def find_record(records: list[IdxRecord], variable: str, level: str, forecast: str) -> IdxRecord:
    for record in records:
        if record.variable == variable and record.level == level and record.forecast == forecast:
            return record
    raise MissingRecordError(f"{variable}:{level}:{forecast} is not in the GRIB index")
...
def sample_sites(
    ...
    try:
        return _sample_gid(gid, sites, max_distance_km)
    finally:
...
def sample_sites_uv(
    message: bytes, sites: list[dict], max_distance_km: float
) -> tuple[dict[str, GridPointValue], dict[str, GridPointValue]]:
    """U and V samples from an NCEP paired-wind GRIB2 message.

    NCEP packs each UGRD/VGRD pair as two submessages of one message (NAM's
    .idx lists them as N.1/N.2 at a shared byte offset), so one ranged fetch
    holds both components. codes_new_from_message decodes only the first
    field; splitting the pair needs ecCodes' multi-field file reader, which
    reads from a real file handle.
    """
    ...
    try:
        ...
        by_component = {}
        for gid in gids:
            # GRIB2 code table 4.2, discipline 0 category 2: 2 = U, 3 = V.
            component = eccodes.codes_get(gid, "parameterNumber")
            by_component[component] = _sample_gid(gid, sites, max_distance_km)
        if 2 not in by_component or 3 not in by_component:
            raise RuntimeError("NOAA paired-wind message is missing a U or V component")
        return by_component[2], by_component[3]
    finally:
        for gid in gids:
            eccodes.codes_release(gid)


def _sample_gid(gid, sites: list[dict], max_distance_km: float) -> dict[str, GridPointValue]:
    """Sampling core shared by the one-field and paired-wind entry points.

    Bitmap-masked gridpoints (NAM nest fields mask ~100 of 1.9M points)
    surface from ecCodes as the in-band missingValue; they are published as

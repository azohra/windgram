...
# precipitation is a run-total accumulation differenced per 3 h step into
# the mean mm/h rate over the window. No gust key — REPS publishes no
...
def _build_documents(
    ...
    # and divided by the window: the surface block publishes mm/h (the gfs
    # builder's idiom). Nothing has accumulated at hour 0 — hour 000
    # publishes no APCP file at all — so the first step's baseline is seeded,
    # not fetched.
...
def _aggregate_hours(member_profiles: list[dict]) -> list[dict]:
    """Aggregate independently derived REPS member profiles."""

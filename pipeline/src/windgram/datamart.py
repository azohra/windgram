...
# The operational Datamart host. WINDGRAM_DATAMART_BASE selects the host per
# invocation: set it to https://hpfx.collab.science.gc.ca to fetch from MSC's
# documented alternative server, which carries the identical date-rooted
# YYYYMMDD/WXO-DD tree (listings and paths verified against dd 2026-08-08)
# with ten-fold peak bandwidth and longer retention, on a best-effort link.
# The experimental HRDPS 1 km feed is NOT covered: it lives on
# dd.alpha.weather.gc.ca outside the dated tree and is not mirrored on hpfx,
# so windgram.builders.hrdps_west ignores this setting.
DD_URL = "https://dd.weather.gc.ca"


def datamart_base() -> str:
    """The Datamart base URL for this invocation, read from the environment
    on every call so tests and the workflow's per-job env both take effect
    without import-order traps."""
    return os.environ.get("WINDGRAM_DATAMART_BASE", DD_URL).rstrip("/")


class NotFoundError(RuntimeError):
    """The Datamart has no file at this URL (HTTP 404). Builders treat this
    as meaningful — e.g. a pressure level absent from a reduced late-horizon
    step — where any other failure stays fatal."""
    ...
    for attempt in range(3):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise NotFoundError(f"Datamart {url} returned 404")

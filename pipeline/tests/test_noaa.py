...
from windgram.noaa import byte_range, find_record, parse_idx, sample_sites, wind_from_uv
...
def test_parses_idx_lines_into_offsets_and_lengths():
    ...
    assert records[0].variable == "VVEL"
    assert records[0].level == "600 mb"
    assert records[0].forecast == "24 hour fcst"
    assert records[0].offset == 150408432
    # A record's length is the next record's offset minus its own.
    assert records[0].length == records[1].offset - records[0].offset == 13631698
...
def test_byte_range_is_inclusive():
    ...
    assert byte_range(records[0]) == "bytes=150408432-164040129"

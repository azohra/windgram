...
from windgram.builders import eccc
from windgram.builders.eccc import (
    GDPS,
...
def test_datamart_builders_share_the_per_host_connection_budget():
    ...
    # ECCC builder on the same budget (see the comment in
    # pipeline/src/windgram/builders/eccc.py).
    from windgram.builders import geps, hrdps_west, reps

    assert eccc.FETCH_CONCURRENCY == 5
    assert hrdps_west.FETCH_CONCURRENCY == eccc.FETCH_CONCURRENCY
    assert reps.FETCH_CONCURRENCY == eccc.FETCH_CONCURRENCY
    assert geps.FETCH_CONCURRENCY == eccc.FETCH_CONCURRENCY
...
def test_every_profile_semantics_declaration_mirrors_the_catalogue():
    ...
    declared = {
        ...
        "hrdps-west": hrdps_west.SEMANTICS,
        "hrrr-conus": hrrr.SEMANTICS,
        "gfs": gfs.SEMANTICS,
        "nam": nam.SEMANTICS,
        "nam-conus-nest": nam.SEMANTICS,
        "reps": reps.SEMANTICS,
        "geps": geps.SEMANTICS,
    }
    catalogue = json.loads(Path("models.json").read_text())
...
def test_build_profiles_end_to_end_against_a_fake_datamart(monkeypatch):
    ...
    monkeypatch.setattr(eccc, "fetch_bytes", fake_fetch)

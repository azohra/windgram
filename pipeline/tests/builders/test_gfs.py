...
from windgram.builders import gfs
from windgram.builders.gfs import (
    OMEGA_LEVELS,
...
def test_models_json_matches_the_gfs_builder_configuration():
    ...
    assert gfs.SEMANTICS == {"gust": "instant", "precipitation": "windowMeanRate"}
...
def test_build_profiles_publishes_omega_and_tolerates_its_absence(monkeypatch):
    ...
    monkeypatch.setattr(gfs, "fetch_record", lambda url, record, stats=None: record)
    monkeypatch.setattr(
        gfs,
        "sample_sites",

...
## The ensemble publishes distributions, not members

The product is percentile spread, never 21 separate windgrams: member spread for cloud base,
boundary-layer top, thermal velocity, and usable-lift top, and — since the Datamart GRIB migration —
the same percentile blocks per pressure level, an ensemble sounding of temperature, moisture, and wind
through the pilot band. Five levels publish; at the catalogued mountain launches the lowest sits below
model terrain in most members and drops out of that member's column, so a level's blocks may count
fewer than 21 members. The
...
ranking, and at sounding levels a member whose model surface sits above the level drops out of that
level's blocks. `ceiledMembers` counts values clamped at the top of the available column, where the reported

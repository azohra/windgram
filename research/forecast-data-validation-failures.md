...
While the pipeline read WCS, the builders compared several crop values with an independent point query
before trusting the georeferencing; the transport is retired, the lesson is not — a crop is a rendered
coverage, not proof of nearest-cell sampling.
...
Each field passed physical plausibility checks from then on. The check matters most when zero can mean
either a valid measurement or a broken request, which is as true of a decoded GRIB field as of a
rendered crop.

...
def test_existing_builder_writes_to_external_output_without_network(tmp_path, monkeypatch):
    from windgram import build_hrrr

    sites = write_sites(tmp_path / "club" / "sites.json")
    output = tmp_path / "public" / "windgrams"
    sampled_sites = []
    monkeypatch.setattr(
        build_hrrr, "_latest_complete_run", lambda: {"date": "20260807", "hour": "12"}
    )
    ...
    monkeypatch.setattr(build_hrrr, "_build_profiles", build_profiles)
...
def make_checkout(root: Path) -> Path:
    ...
    (root / "packages" / "windgram" / "schema").mkdir(parents=True)
    return root
...
def test_hrdps_west_max_steps_preserves_default_schedule(monkeypatch):
    from windgram import build_1km

    monkeypatch.delenv("WINDGRAM_MAX_STEPS", raising=False)
    assert build_1km._forecast_hours() == tuple(range(1, build_1km.FORECAST_HOURS + 1))

    monkeypatch.setenv("WINDGRAM_MAX_STEPS", "2")
    assert build_1km._forecast_hours() == (1, 2)


def test_direct_module_output_defaults_are_unchanged():
    from windgram import build, build_1km, build_geps, build_gfs, build_hrrr, build_reps

    assert build.HRDPS.out_dir == Path("data/hrdps-continental")
    assert build.RDPS.out_dir == Path("data/rdps")
    assert build.GDPS.out_dir == Path("data/gdps")
    assert build_1km._out_dir() == Path("data/hrdps-west")
    assert build_hrrr._out_dir() == Path("data/hrrr-conus")
    assert build_gfs._out_dir() == Path("data/gfs")
    assert build_reps._out_dir() == Path("data/reps")
    assert build_geps._out_dir() == Path("data/geps")
